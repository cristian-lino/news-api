...
const index = async (req, res) => {
    try {
        const data = await CreatorRequisition.findAll({ where: { status: 'PENDING' } })
...
const create = async (req, res) => {
    try {
        ...
        const requisition = await CreatorRequisition.findOne({ where: { user_id: userId } })
        if (requisition) {
            return res.status(422).json({ message: `user ${userId} already has a reservation` })
        }
...
const update = async (req, res) => {
    try {
        ...
        if (!requisition) {
            return res.status(422).json({ message: `requisition ${id} does not exists` })
        }
